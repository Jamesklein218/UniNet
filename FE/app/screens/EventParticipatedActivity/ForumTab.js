...
import {user, profile} from '../../data/fake';
      ...
      <SafeAreaView
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <ForumItem
              image={PostData[index % 3].image}
              title={item.title}
              description={item.content}
              item={item}
              onPress={() => navigation.navigate('ForumDetail', {item, index})}>
              <ProfileAuthor
                image={profile[index % profile.length]}
                name={user[index % user.length]}
                username={user[index % user.length]}
